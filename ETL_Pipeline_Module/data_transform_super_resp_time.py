...
def create_resp_time_model_table(log_df, path):
    ...
    log_df.datetime = pd.to_datetime(
        log_df.datetime, format="%Y-%m-%d %H:%M:%S"
    )
    ...
    app_cols = [
        col for col in log_df.columns if "app_" in col
    ]
    resp_codes = [
        col for col in log_df.columns if "resp_code" in col
    ]
    ...
    cumsum_cols = [
        col for col in log_df.columns if "CS_" in col
    ]

    # Create rolling average request response time (over the next 250 requests)

    log_df["av_rolling_resp_time_250"] = (
        log_df["resp_time"].rolling(250).sum() / 250
    )
    log_df.dropna(inplace=True)
    ...
    log_df["av_rolling_resp_time_250"] = log_df[
        "av_rolling_resp_time_250"
    ].shift(-250)
    log_df.dropna(inplace=True)

    # Scale key cols between 0 and 1

    log_df[cumsum_cols] = MinMaxScaler().fit_transform(
        log_df[cumsum_cols]
    )
    log_df[
        "av_rolling_resp_time_250"
    ...
    print(
        "Response time modelling table saved at path: "
        + path
    )
