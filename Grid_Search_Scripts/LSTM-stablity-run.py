...
y_val = array(
    test_df[
        [col for col in test_df.columns if "CS_" in col]
    ]
).reshape(1120000, 129)

X_val = test_df[
    [col for col in test_df.columns if "CS_" not in col]
].to_numpy()
...
y = array(
    train_df[
        [col for col in train_df.columns if "CS_" in col]
    ]
).reshape(2241000, 129)

X = train_df[
    [col for col in train_df.columns if "CS_" not in col]
].to_numpy()
...
y_test = array(
    holdout_df[
        [col for col in holdout_df.columns if "CS_" in col]
    ]
).reshape(1120000, 129)

X_test = holdout_df[
    [col for col in holdout_df.columns if "CS_" not in col]
].to_numpy()
...
for i in range(5):
    ...
    # compile model
    model.compile(
        loss="mae", optimizer=keras.optimizers.Adam(lr=0.01)
    )
    # fit model
    history = model.fit(
        X,
        ...
        batch_size=128,
        epochs=1,
        validation_data=(X_val, sub_y_val),
        shuffle=False,
    )
    # story history
    ...
    app_bias_list = get_app_bias_error(
        X_test, sub_y_test, model
    )
...
plt.plot(
    app_bias_error.T, color="red", label="app_bias_error"
)
plt.legend()
